...
$(function() {
  ...
  function step3(room) {
    ...
    const header = $('<div class=line__title></div>').html('<input type="submit" class="button" value="EXIT" onclick="window.location.reload();" /></div><strong>' + room.name + '</strong>');
    const messages = $('<div class="line__right"><div><em>Peer connected.</em></div></div>').addClass('messages');
    ...
    $('#sendtextform').on('submit', e => {
      ...
      // ルームに送って自分のところにも反映
      room.send(msg);
      messages.prepend('<div class="line__right">  <div class="text"><div><span class="you">You: </span>' + msg + '</div></div></div>');
      $('#mymessage').val('');
    });
